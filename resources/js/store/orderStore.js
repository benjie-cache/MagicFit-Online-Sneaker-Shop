...
async handleBillingInfo() {
  ...
  try {
    ...
    if (response.status === 200) {
      // If the status is 200, move to the next step
      this.updateStep('paymentInfo');
      console.log('Information saved successfully');
      showNotification("success","Billing Information has been updated.You can proceed to payment","success")
    } else {
      // Handle other status codes or error scenarios
      console.error('Failed to save information. Status:', response.status);
